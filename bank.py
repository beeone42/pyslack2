...
def balance(message, login):
    if (my_chan(message)):
        r = requests.get(config['bank_base'] + "balance.php?login=" + login + "&key=" + config['bank_key'])
        message.send(r.content)

@listen_to('^ops ([a-zA-Z0-9_-]+)$')
def balance(message, login):
    if (my_chan(message)):
        r = requests.get(config['bank_base'] + "ops.php?login=" + login + "&key=" + config['bank_key'])
        message.send(r.content)

@listen_to('^credit ([a-zA-Z0-9_-]+) ([0-9.]+)$')
def credit(message, login, amount):
    if (my_chan(message)):
        r = requests.get(config['bank_base'] + "credit.php?login=" + login + "&amount=" + amount + "&key=" + config['bank_key'])
        message.send(r.content)

@listen_to('^pay ([a-zA-Z0-9_-]+) ([0-9.]+)$')
def pay(message, login, amount):
    if (my_chan(message)):
        r = requests.get(config['bank_base'] + "pay.php?login=" + login + "&amount=" + amount + "&key=" + config['bank_key'])
        message.send(r.content)
